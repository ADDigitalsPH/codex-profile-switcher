...
Branded for Agentic Revenue Vision.
...
9. To add another account later, click "Prepare new account login". The switcher asks for the new profile name in a pop-up window, saves the current active auth, closes Codex if needed, clears only the active auth files in %USERPROFILE%\.codex, and marks the new empty profile active.
10. Open Codex and log into the other account manually. Do not log out of the previous account first.
11. After login completes, return to the switcher and click "Save current profile now" to save the new account auth into its active profile.
12. Select a profile, then click "Switch to selected". The app will close Codex.exe first if it is open, restore the selected auth, then try to re-open Codex automatically.
13. To rename a saved profile, select an inactive profile and click "Rename profile". The switcher asks for the new name in a pop-up window. The active profile cannot be renamed.
14. To delete a saved profile, select an inactive profile and click "Delete selected profile". The switcher requires you to type the profile name before deletion.
...
- Do not use Codex logout to create another profile. Logout may revoke the token in the current profile. Use "Prepare new account login" instead.
...
- The main action buttons are ordered as: Prepare new account login, Save current profile now, Switch to selected, Rename profile, Delete selected profile.
